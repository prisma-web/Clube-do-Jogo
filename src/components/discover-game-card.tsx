...
import { Check, Clock3, Library, MoreHorizontal, Star, Trophy, Users } from 'lucide-react';
    ...
    <article className="discover-game-card group relative min-w-0 overflow-hidden rounded-2xl border border-white/[0.08] bg-white/[0.03]">
      ...
      <div className="discover-poster relative aspect-[3/4] overflow-hidden bg-zinc-900">
        ...
        <div className="pointer-events-none absolute inset-x-2 bottom-2 flex items-end justify-between gap-2 text-[9px] font-bold text-white/85">
          ...
          {game.average_rating != null && <span className="inline-flex items-center gap-1 rounded-full bg-black/65 px-2 py-1 text-amber-300 backdrop-blur"><Star className="size-3 fill-current" />{(game.average_rating / 10).toLocaleString('pt-BR', { maximumFractionDigits: 1 })}</span>}
        </div>
      </div>
      <footer className="favorite-game-footer flex min-h-9 min-w-0 items-center gap-1.5 px-2 py-1.5 text-[9px] font-bold text-zinc-500"><span className="inline-flex items-center gap-1"><Clock3 className="size-3" />{game.duration_hours} h</span>{!!item.people?.length && <span className="min-w-0 flex-1 truncate text-right">{item.people[0]}{item.people.length > 1 ? ` +${item.people.length - 1}` : ''}</span>}</footer>
