  ...
  return (
    <div className="mx-auto max-w-3xl animate-fade-in">
      <section className="profile-hero relative mb-7 rounded-[30px] border border-white/8 bg-white/[0.025] p-6 text-center">
        ...
        {own && <PushNotificationButton className="profile-hero-action absolute left-16 top-4 z-10" />}
        {own && <Dialog open={themeDialog.open} onOpenChange={open => open ? themeDialog.show() : themeDialog.close()}><DialogTrigger asChild><button aria-label="Escolher tema" title="Escolher tema" className="profile-hero-action profile-theme-action absolute left-4 top-4 z-10 grid size-10 place-items-center rounded-xl border border-violet-400/20 bg-violet-500/10 text-violet-200 transition hover:bg-violet-500/20 hover:text-white"><Palette className="size-5" /></button></DialogTrigger><DialogContent title="Tema visual" description="Escolha o tema salvo neste dispositivo."><div className="p-5"><ThemeSelector compact /></div></DialogContent></Dialog>}
        ...
        <div className="profile-stat-list mt-4 flex flex-wrap items-center justify-center gap-2 text-[10px] font-bold text-zinc-500"><span className="profile-stat inline-flex items-center gap-1.5 whitespace-nowrap rounded-full bg-white/5 px-2.5 py-1.5"><Library className="size-3" />{data.backlog.length} no backlog</span><span className="profile-stat inline-flex items-center gap-1.5 whitespace-nowrap rounded-full bg-white/5 px-2.5 py-1.5"><Flag className="size-3" />{data.completed.length} finalizados</span><span className="profile-stat inline-flex items-center gap-1.5 whitespace-nowrap rounded-full bg-white/5 px-2.5 py-1.5"><Gamepad2 className="size-3" />{data.platforms.length} consoles</span></div>
      ...
      {own ? <YourGamesPanel embedded /> : <Tabs.Root value={activeTab} onValueChange={value => setActiveTab(value as typeof activeTab)}>
        ...
        <Tabs.Content value="platforms" className="outline-none data-[state=active]:animate-fade-in">{data.platforms.length ? <section className="profile-platforms-surface rounded-3xl border border-white/8 bg-white/[.025] p-4 sm:p-5"><div className="flex flex-wrap gap-2.5">{data.platforms.map(platform => <span key={platform.igdb_platform_id} className="profile-platform-chip inline-flex min-h-10 max-w-full items-center gap-2 rounded-full border border-cyan-400/15 bg-cyan-500/[.07] px-3 py-1.5 text-xs font-bold text-cyan-100"><span className="truncate">{platform.name}</span></span>)}</div></section> : <Empty text="Nenhum console adicionado." />}</Tabs.Content>
      </Tabs.Root>}

      {own && isAdmin && <Link href="/admin" className="admin-entry profile-admin-entry mb-7 flex items-center gap-3 rounded-2xl border border-violet-400/15 bg-violet-500/[0.06] p-4 transition hover:bg-violet-500/10"><span className="admin-entry-icon grid size-10 shrink-0 place-items-center rounded-xl bg-violet-500/12 text-violet-300"><ShieldCheck className="size-4" /></span><span className="min-w-0 flex-1"><strong className="block text-sm">Cargos e acesso</strong><span className="admin-entry-copy mt-0.5 block text-[11px] text-zinc-500">Gerencie administradores e usuários comuns.</span></span><ChevronRight className="admin-entry-arrow size-4 shrink-0 text-zinc-600" /></Link>}
