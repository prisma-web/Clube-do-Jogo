...
export function ProfileView({ profileId, own = false }: { profileId: string; own?: boolean }) {
  ...
  return (
    <div className="mx-auto max-w-3xl animate-fade-in">
      ...
      {own && <><ThemeSelector /><div className="mt-5 border-t border-white/8 pt-5"><button onClick={() => void signOut()} className="danger-action inline-flex h-11 w-full items-center justify-center gap-2 whitespace-nowrap rounded-xl border border-red-500/15 bg-red-500/[0.06] px-4 text-xs font-extrabold text-red-300 transition hover:bg-red-500/10"><LogOut className="size-4" />Sair da conta</button></div></>}
    </div>
  );
}
