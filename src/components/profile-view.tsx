  ...
  return (
    <div className="mx-auto max-w-3xl animate-fade-in">
      <section className="profile-hero relative mb-7 overflow-hidden rounded-[30px] border border-white/8 bg-white/[0.025] p-6 text-center">
        ...
        {person.created_at && <div className="profile-since mt-4 flex justify-center text-[10px] font-bold text-zinc-500"><span className="inline-flex items-center gap-1.5 whitespace-nowrap"><CalendarDays className="size-3" />Desde {formatDate(person.created_at)}</span></div>}
      </section>
      ...
      <Tabs.Root value={activeTab} onValueChange={value => setActiveTab(value as typeof activeTab)}>
        ...
        <Tabs.Content value="favorites" className="outline-none data-[state=active]:animate-fade-in">{data.favorites.length ? <div ref={favoritesParent} className="poster-game-grid grid gap-2.5 sm:gap-3">{data.favorites.map(game => <FavoriteGameCard key={game.id} game={game} />)}</div> : <Empty text="Nenhum jogo favorito." />}</Tabs.Content>
