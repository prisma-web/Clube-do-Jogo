...
import { CalendarCheck2, CalendarClock, CheckCircle2, ChevronDown, Circle, PlayCircle } from 'lucide-react';
...
import { RatingDisplay, RatingSlider } from './rating-slider';
    ...
    <div className="space-y-4">
      <section className="progress-status-card rounded-2xl border border-white/8 bg-white/[0.025] p-4">
        ...
        {mine?.status === 'finished' && <div className="mt-4 space-y-4 border-t border-white/8 pt-4">
          <div className="flex flex-wrap items-center justify-between gap-3"><span className="block text-xs font-bold text-zinc-400">Minha nota</span>{mine.rating_mode === 'detailed' && <RatingDisplay value={mine.rating ?? 0} className="text-sm" />}</div>
