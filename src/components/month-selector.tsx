...
export function MonthSelector() {
  ...
  return (
    <DropdownMenu.Root>
      <DropdownMenu.Trigger className="month-selector-trigger inline-flex max-w-full items-center gap-2 whitespace-nowrap rounded-full border border-white/10 bg-white/5 px-3 py-2 text-xs font-bold text-zinc-200 outline-none transition hover:bg-white/10 data-[state=open]:bg-white/10">
      ...
      <DropdownMenu.Portal>
        <DropdownMenu.Content align="end" sideOffset={8} collisionPadding={12} className="month-popup app-popup animated-popup z-[100] min-w-56 rounded-2xl border border-white/10 bg-zinc-900 p-1.5 shadow-2xl outline-none">
          <div className="px-3 py-2 text-[10px] font-black uppercase tracking-[0.18em] text-zinc-500">Mês da atividade</div>
          {availableMonths.map(month => (
            <DropdownMenu.Item key={month} data-selected={month === selectedMonth} onSelect={() => setSelectedMonth(month)} className="month-popup-item flex cursor-pointer items-center justify-between rounded-xl px-3 py-2.5 text-sm text-zinc-300 outline-none transition data-[highlighted]:bg-white/8 data-[highlighted]:text-white">
