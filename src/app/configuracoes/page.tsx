...
export default function SettingsPage() {
  ...
  const [activeTab, setActiveTab] = useUrlTab('section', tabs.map(tab => tab.value), 'preferences');
...
function MotionPreference() {
  ...
  return <label className="flex min-h-14 items-center justify-between gap-4 pt-3 text-sm font-bold"><span>Reduzir movimento</span><input type="checkbox" checked={enabled} className="size-5 accent-violet-500" onChange={event => { const value = event.target.checked; document.documentElement.dataset.reduceMotion = value ? 'true' : 'false'; localStorage.setItem('clube-do-jogo:reduce-motion', String(value)); window.dispatchEvent(new Event(motionEvent)); }} /></label>;
}

function RatingScalePreference() {
  const [scale, setScale] = useRatingScale();
  return <div className="pb-4"><span className="mb-2 block text-sm font-bold">Escala das notas</span><div className="grid grid-cols-2 gap-2" role="radiogroup" aria-label="Escala das notas"><button type="button" role="radio" aria-checked={scale === 10} onClick={() => setScale(10)} data-selected={scale === 10} className="library-filter-chip flex h-11 items-center justify-center gap-2 rounded-xl border text-xs font-extrabold"><span className="text-base tabular-nums">10</span> pontos</button><button type="button" role="radio" aria-checked={scale === 5} onClick={() => setScale(5)} data-selected={scale === 5} className="library-filter-chip flex h-11 items-center justify-center gap-2 rounded-xl border text-xs font-extrabold"><span className="inline-flex text-amber-400"><Star className="size-4 fill-current" /></span>5 estrelas</button></div></div>;
}
