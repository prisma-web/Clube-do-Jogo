        ...
        <div ref={rankingParent} className="space-y-3 pl-2 pt-2">
          {visibleRanking.map((item, index) => (
            <article key={item.game.id} className={`relative min-w-0 rounded-2xl border p-3 transition ${index === 0 ? 'border-amber-400/25 bg-gradient-to-br from-amber-400/[.07] to-white/[.025]' : 'border-white/[.07] bg-white/[.025]'}`}>
              ...
              {!isHistorical && <DropdownMenu.Root><DropdownMenu.Trigger aria-label={`Opções de ${item.game.title}`} className="absolute right-2 top-2 z-20 grid size-8 place-items-center rounded-full bg-zinc-950/75 text-zinc-400 backdrop-blur hover:bg-zinc-800 hover:text-white"><MoreHorizontal className="size-4" /></DropdownMenu.Trigger><DropdownMenu.Portal><DropdownMenu.Content align="end" sideOffset={6} className="z-[100] min-w-52 rounded-xl border border-white/10 bg-zinc-900 p-1.5 shadow-2xl outline-none data-[state=closed]:animate-pop-out data-[state=open]:animate-pop-in"><DropdownMenu.Item disabled={item.inBacklog} onSelect={() => void addToBacklog(item)} className="flex items-center gap-2 rounded-lg px-3 py-2.5 text-xs font-bold text-zinc-300 outline-none data-[disabled]:text-emerald-400 data-[highlighted]:bg-white/8">{item.inBacklog ? <Check className="size-3.5" /> : <Library className="size-3.5" />}{item.inBacklog ? 'Jogo já no Backlog' : 'Adicionar jogo ao Backlog'}</DropdownMenu.Item></DropdownMenu.Content></DropdownMenu.Portal></DropdownMenu.Root>}
