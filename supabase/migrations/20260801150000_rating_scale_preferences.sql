-- Production delta for the preference voting and 0-10 rating release.
-- Existing rows are preserved. Values already stored on the 0-10 scale are
-- not converted a second time.
...
ALTER TABLE public.game_progress
  ADD CONSTRAINT game_progress_rating_range_check
...
ALTER TABLE public.cycle_progress_snapshots
  ADD CONSTRAINT cycle_progress_snapshots_rating_range_check
  CHECK (rating IS NULL OR rating BETWEEN 0 AND 10);
...
BEGIN
  WITH preference_counts AS (
    ...
    SELECT *, DENSE_RANK() OVER (ORDER BY total_points DESC)::INTEGER AS position
    FROM scores
  )
